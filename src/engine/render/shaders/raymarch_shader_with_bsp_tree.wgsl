...
{
let intr = cube_intersection(
                ro - vec4<f32>(-6, 2.5, 50, 0),
                rd,
                vec4<f32>(1.88, 2.58, 0.28, 15.08)
            );
if intr.y > 0.0 {
            store_intersection_entrance_and_exit(intr, intrs);
        }
...
{
let intr = cube_intersection(
                ro - vec4<f32>(-6, 4.9, 38.5, 0),
                rd,
                vec4<f32>(1.88, 0.26, 15.58, 1.08)
            );
if intr.y > 0.0 {
            store_intersection_entrance_and_exit(intr, intrs);
        }
...
let intr = cube_intersection(
                ...
                rd,
                vec4<f32>(2.8000002, 2.8000002, 23.4, 15.6)
            );
if intr.y > 0.0 {
            store_intersection_entrance_and_exit(intr, intrs);
        }
...
{
let intr = cube_intersection(
                ro - vec4<f32>(11, 1.2, 37, 0),
                rd,
                vec4<f32>(2.911239, 1.861239, 1.611239, 2.4612389)
            );
if intr.y > 0.0 {
            store_intersection_entrance_and_exit(intr, intrs);
        }
...
{
let intr = cube_intersection(
                ro - vec4<f32>(-6, 0, 40, 0),
                rd,
                vec4<f32>(2.351239, 0.75123894, 10.551239, 15.551239)
            );
if intr.y > 0.0 {
            store_intersection_entrance_and_exit(intr, intrs);
        }
...
d = smin(d, sd_box(p - vec4<f32>(11, 1.2, 37, 0), vec4<f32>(2.4, 1.35, 1.1, 1.95)) - 0.04, 0.15);
for (var i = dynamic_data.shapes_arrays_metadata.neg_spheres_start; i < dynamic_data.shapes_arrays_metadata.neg_spheres_start + dynamic_data.shapes_arrays_metadata.neg_spheres_amount; i++) {
...
{d = min(d, sd_box(p - vec4<f32>(4, -0.7, 35, 0), vec4<f32>(13, 0.1, 12, 15)) - 0.6);
d = max(d, -(sd_box(p - vec4<f32>(3, 4, 37, 3.35), vec4<f32>(2.4, 2, 2.6, 1)) - 0.08));
for (var i = dynamic_data.shapes_arrays_metadata.neg_spheres_start; i < dynamic_data.shapes_arrays_metadata.neg_spheres_start + dynamic_data.shapes_arrays_metadata.neg_spheres_amount; i++) {
                    d = max(d, -(sd_sphere(p - dyn_negatives_shapes[i].pos, dyn_negatives_shapes[i].size.x) - dyn_negatives_shapes[i].roundness));
                }
...
{d = min(d, sd_box(p - vec4<f32>(4, -0.7, 35, 0), vec4<f32>(13, 0.1, 12, 15)) - 0.6);
d = min(d, sd_box(p - vec4<f32>(8, 0.5, 0, 17), vec4<f32>(1.2, 1.2, 21.8, 14)) - 1.6);
d = smin(d, sd_box(p - vec4<f32>(13, 0.2, 37, 0), vec4<f32>(3.4, 0.7, 7, 0.8)) - 0.3, 0.15);
for (var i = dynamic_data.shapes_arrays_metadata.neg_spheres_start; i < dynamic_data.shapes_arrays_metadata.neg_spheres_start + dynamic_data.shapes_arrays_metadata.neg_spheres_amount; i++) {
                    d = max(d, -(sd_sphere(p - dyn_negatives_shapes[i].pos, dyn_negatives_shapes[i].size.x) - dyn_negatives_shapes[i].roundness));
                }
d = smax(d, -(sd_box(p - vec4<f32>(8, 1.9, 25, 11.5), vec4<f32>(10, 2.2, 6, 10)) - 0.3), 0.15);
d = smax(d, -(sd_box(p - vec4<f32>(8, 0.5, 0, 3), vec4<f32>(0.6, 0.6, 30, 0.001)) - 1), 0.15);
d = min(d, sd_box(p - vec4<f32>(0, 0, 0, -1), vec4<f32>(50, 50, 100, 1)) - 0.08);
}}
else
{d = min(d, sd_box(p - vec4<f32>(4, -0.7, 35, 0), vec4<f32>(13, 0.1, 12, 15)) - 0.6);
d = smin(d, sd_box(p - vec4<f32>(3, 1, 37, 0), vec4<f32>(1.4, 1.4, 1.4, 5)) - 0.08, 0.15);
...
{d = min(d, sd_box(p - vec4<f32>(4, -0.7, 35, 0), vec4<f32>(13, 0.1, 12, 15)) - 0.6);
d = smin(d, sd_box(p - vec4<f32>(-6, 0, 40, 0), vec4<f32>(1.8, 0.2, 10, 15)) - 0.08, 0.15);
for (var i = dynamic_data.shapes_arrays_metadata.neg_spheres_start; i < dynamic_data.shapes_arrays_metadata.neg_spheres_start + dynamic_data.shapes_arrays_metadata.neg_spheres_amount; i++) {
                    d = max(d, -(sd_sphere(p - dyn_negatives_shapes[i].pos, dyn_negatives_shapes[i].size.x) - dyn_negatives_shapes[i].roundness));
                }
d = smax(d, -(sd_box(p - vec4<f32>(8, 1.9, 25, 11.5), vec4<f32>(10, 2.2, 6, 10)) - 0.3), 0.15);
...
{if p.y > 1.7 {
d = min(d, sd_box(p - vec4<f32>(-6, 2.5, 50, 0), vec4<f32>(1.8, 2.5, 0.2, 15)) - 0.08);
d = min(d, sd_box(p - vec4<f32>(-6, 4.9, 38.5, 0), vec4<f32>(1.8, 0.18, 15.5, 1)) - 0.08);
for (var i = dynamic_data.shapes_arrays_metadata.neg_spheres_start; i < dynamic_data.shapes_arrays_metadata.neg_spheres_start + dynamic_data.shapes_arrays_metadata.neg_spheres_amount; i++) {
                    d = max(d, -(sd_sphere(p - dyn_negatives_shapes[i].pos, dyn_negatives_shapes[i].size.x) - dyn_negatives_shapes[i].roundness));
                }
d = smax(d, -(sd_box(p - vec4<f32>(-1, 5.2, 37, 4.35), vec4<f32>(3, 2, 3, 2)) - 0.08), 0.15);
...
d = smax(d, -(sd_box(p - vec4<f32>(4.05, 5.3, 0, 10), vec4<f32>(10.2, 0.2, 1.1, 8)) - 1), 0.15);
d = min(d, sd_box(p - vec4<f32>(0, 0, 0, -1), vec4<f32>(50, 50, 100, 1)) - 0.08);
}
else
...
{d = min(d, sd_box(p - vec4<f32>(5.5, 3.8, 0, 10), vec4<f32>(6.5, 0.2, 2.2, 10)) - 0.08);
d = min(d, sd_sph_box(p - vec4<f32>(-6, 5.3, 0, 5.1), vec4<f32>(2, 2, 2, 1.5)) - 3.5);
d = max(d, -(sd_sphere(p - vec4<f32>(-6, 5.3, 0, 4), 3.5) - 0));
for (var i = dynamic_data.shapes_arrays_metadata.neg_spheres_start; i < dynamic_data.shapes_arrays_metadata.neg_spheres_start + dynamic_data.shapes_arrays_metadata.neg_spheres_amount; i++) {
                    d = max(d, -(sd_sphere(p - dyn_negatives_shapes[i].pos, dyn_negatives_shapes[i].size.x) - dyn_negatives_shapes[i].roundness));
                }
d = smax(d, -(sd_box(p - vec4<f32>(-6, 5.3, 0, 3), vec4<f32>(0.5, 0.5, 8, 0.5)) - 0.6), 0.15);
d = smax(d, -(sd_box(p - vec4<f32>(4.05, 5.3, 0, 10), vec4<f32>(10.2, 0.2, 1.1, 8)) - 1), 0.15);
d = min(d, sd_box(p - vec4<f32>(0, 0, 0, -1), vec4<f32>(50, 50, 100, 1)) - 0.08);
}}}
else
...
{
let dd = min(d, sd_box(p - vec4<f32>(11, 1.2, 37, 0), vec4<f32>(2.4, 1.35, 1.1, 1.95)) - 0.04);
if dd < MIN_DIST*2.0 {
                        output.materials_count = 1u;
                        output.material_weights[0] = 1.0;
                        output.materials[0] = 5;
...
{
let dd = min(d, sd_box(p - vec4<f32>(8, 0.5, 0, 17), vec4<f32>(1.2, 1.2, 21.8, 14)) - 1.6);
if dd < MIN_DIST*2.0 {
                        output.materials_count = 1u;
                        output.material_weights[0] = 1.0;
                        output.materials[0] = 5;
...
{
let dd = min(d, sd_box(p - vec4<f32>(3, 1, 37, 0), vec4<f32>(1.4, 1.4, 1.4, 5)) - 0.08);
if dd < MIN_DIST*2.0 {
                        output.materials_count = 1u;
                        output.material_weights[0] = 1.0;
                        output.materials[0] = 5;
...
{
let dd = min(d, sd_box(p - vec4<f32>(-6, 0, 40, 0), vec4<f32>(1.8, 0.2, 10, 15)) - 0.08);
if dd < MIN_DIST*2.0 {
                        output.materials_count = 1u;
                        output.material_weights[0] = 1.0;
                        output.materials[0] = 1;
...
{
let dd = min(d, sd_box(p - vec4<f32>(-6, 2.5, 50, 0), vec4<f32>(1.8, 2.5, 0.2, 15)) - 0.08);
if dd < MIN_DIST*2.0 {
                        output.materials_count = 1u;
                        output.material_weights[0] = 1.0;
                        output.materials[0] = 1;
...
{
let dd = min(d, sd_box(p - vec4<f32>(-6, 4.9, 38.5, 0), vec4<f32>(1.8, 0.18, 15.5, 1)) - 0.08);
if dd < MIN_DIST*2.0 {
                        output.materials_count = 1u;
                        output.material_weights[0] = 1.0;
                        output.materials[0] = 1;
...
{
let dd = min(d, sd_sph_box(p - vec4<f32>(-6, 5.3, 0, 5.1), vec4<f32>(2, 2, 2, 1.5)) - 3.5);
if dd < MIN_DIST*2.0 {
                        output.materials_count = 1u;
                        output.material_weights[0] = 1.0;
                        output.materials[0] = 1;
