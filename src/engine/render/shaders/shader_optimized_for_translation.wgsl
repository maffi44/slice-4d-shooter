...
fn get_mat(
    ...
    ray_dir: vec4<f32>,
    dist: f32,
    ismd: ptr<function, IntersectedShapesMetadata>,
    ish: ptr<function, array<u32, 16>>,
    mats: ptr<function, array<i32,32>>,
    mats_wieghts: ptr<function, array<f32,32>>,
    mat_count: ptr<function,u32>
) {
    ...
    *mat_count = 0u;
    ...
    for (var j = ismda.player_forms_start; j < ismda.player_forms_amount + ismda.player_forms_start; j++) {
        ...
        let shape = dyn_player_forms[ (*ish)[j] ];
        ...
        if d < MIN_DIST {
            *mat_count = 1u;
            (*mats_wieghts)[0] = 1.0;
            (*mats)[0] = static_data.players_mat1;
            return;
        }
        ...
        if d < MIN_DIST {
            *mat_count = 1u;
            (*mats_wieghts)[0] = 1.0;
            (*mats)[0] = static_data.players_mat2;
            return;
        }
        ...
        if dd < static_data.stickiness * STICKINESS_EFFECT_COEF {
            if *mat_count == 0u {
                *mat_count = 1u;
                ...
                (*mats)[*mat_count] = shape.material;
                (*mats_wieghts)[*mat_count] = coef;

                let mult = 1.0 - coef;

                for (var k = 0u; k < *mat_count; k++) {
                    (*mats_wieghts)[k] *= mult;
                }
...
fn get_normal(
    ...
    var fa: f32 = map(a, ismd, ish);
    var fb: f32 = map(b, ismd, ish);
    var fc: f32 = map(c, ismd, ish);
    var fd: f32 = map(d, ismd, ish);
    var fe: f32 = map(e, ismd, ish);
    var ff: f32 = map(f, ismd, ish);
...
fn ray_march(
    ...
    var i: i32 = 0;
    for (; i < MAX_STEPS; i++) {
        var d: f32  = map(ray_origin, ismd, ish);
        total_distance += d;
...
fn find_intersections(
    ...
    (*ismd).st_s_cubes_start = ish_index;
    ...
    (*ismd).st_s_spheres_start = ish_index;
    ...
    (*ismd).st_s_sph_cubes_start = ish_index;
    ...
    (*ismd).st_s_inf_cubes_start = ish_index;
    ...
    (*ismd).dyn_s_cubes_start = ish_index;
    ...
    (*ismd).dyn_s_spheres_start = ish_index;
    ...
    (*ismd).dyn_s_sph_cubes_start = ish_index;
    ...
    for (var i = dynamic_data.shapes_arrays_metadata.s_sph_cubes_start; i < dynamic_data.shapes_arrays_metadata.s_sph_cubes_amount + dynamic_data.shapes_arrays_metadata.s_sph_cubes_start; i++) {
        ...
        if intr.y > 0.0 {
            (*ismd).dyn_s_sph_cubes_amount += 1u;
    ...
    (*ismd).dyn_s_inf_cubes_start = ish_index;
    ...
    for (var i = dynamic_data.shapes_arrays_metadata.s_inf_cubes_start; i < dynamic_data.shapes_arrays_metadata.s_inf_cubes_amount + dynamic_data.shapes_arrays_metadata.s_inf_cubes_start; i++) {
        ...
        if intr.y > 0.0 {
            (*ismd).dyn_s_inf_cubes_amount += 1u;
    ...
    (*ismd).st_spheres_start = ish_index;
    ...
    (*ismd).st_sph_cubes_start = ish_index;
    ...
    (*ismd).st_inf_cubes_start = ish_index;
    ...
    (*ismd).dyn_cubes_start = ish_index;
    ...
    (*ismd).dyn_spheres_start = ish_index;
    ...
    (*ismd).dyn_sph_cubes_start = ish_index;
    ...
    (*ismd).dyn_inf_cubes_start = ish_index;
    ...
    // static negative stickiness shapes
    (*ismd).st_s_neg_cubes_start = ish_index;
    ...
    (*ismd).st_s_neg_spheres_start = ish_index;
    ...
    for (var i = static_data.shapes_arrays_metadata.s_neg_spheres_start; i < static_data.shapes_arrays_metadata.s_neg_spheres_amount + static_data.shapes_arrays_metadata.s_neg_spheres_start; i++) {
        ...
        if intr.y > 0.0 {
            (*ismd).st_s_neg_spheres_amount += 1u;
    ...
    (*ismd).st_s_neg_sph_cubes_start = ish_index;
    ...
    for (var i = static_data.shapes_arrays_metadata.s_neg_sph_cubes_start; i < static_data.shapes_arrays_metadata.s_neg_sph_cubes_amount + static_data.shapes_arrays_metadata.s_neg_sph_cubes_start; i++) {
        ...
        if intr.y > 0.0 {
            (*ismd).st_s_neg_sph_cubes_amount += 1u;
    ...
    (*ismd).st_s_neg_inf_cubes_start = ish_index;
    ...
    for (var i = static_data.shapes_arrays_metadata.s_neg_inf_cubes_start; i < static_data.shapes_arrays_metadata.s_neg_inf_cubes_amount + static_data.shapes_arrays_metadata.s_neg_inf_cubes_start; i++) {
        ...
        if intr.y > 0.0 {
            (*ismd).st_s_neg_inf_cubes_amount += 1u;
    ...
    // dynamic negative stickiness shapes
    (*ismd).dyn_s_neg_cubes_start = ish_index;
    ...
    for (var i = dynamic_data.shapes_arrays_metadata.s_neg_cubes_start; i < dynamic_data.shapes_arrays_metadata.s_neg_cubes_amount + dynamic_data.shapes_arrays_metadata.s_neg_cubes_start; i++) {
        ...
        if intr.y > 0.0 {
            (*ismd).dyn_s_neg_cubes_amount += 1u;
    ...
    (*ismd).dyn_s_neg_spheres_start = ish_index;
    ...
    for (var i = dynamic_data.shapes_arrays_metadata.s_neg_spheres_start; i < dynamic_data.shapes_arrays_metadata.s_neg_spheres_amount + dynamic_data.shapes_arrays_metadata.s_neg_spheres_start; i++) {
        ...
        if intr.y > 0.0 {
            (*ismd).dyn_s_neg_spheres_amount += 1u;
    ...
    (*ismd).dyn_s_neg_sph_cubes_start = ish_index;
    ...
    for (var i = dynamic_data.shapes_arrays_metadata.s_neg_sph_cubes_start; i < dynamic_data.shapes_arrays_metadata.s_neg_sph_cubes_amount + dynamic_data.shapes_arrays_metadata.s_neg_sph_cubes_start; i++) {
        ...
        if intr.y > 0.0 {
            (*ismd).dyn_s_neg_sph_cubes_amount += 1u;
    ...
    (*ismd).dyn_s_neg_inf_cubes_start = ish_index;
    ...
    for (var i = dynamic_data.shapes_arrays_metadata.s_neg_inf_cubes_start; i < dynamic_data.shapes_arrays_metadata.s_neg_inf_cubes_amount + dynamic_data.shapes_arrays_metadata.s_neg_inf_cubes_start; i++) {
        ...
        if intr.y > 0.0 {
            (*ismd).dyn_s_neg_inf_cubes_amount += 1u;
    ...
    (*ismd).st_neg_cubes_start = ish_index;
    ...
    (*ismd).st_neg_spheres_start = ish_index;
    ...
    (*ismd).st_neg_sph_cubes_start = ish_index;
    ...
    for (var i = static_data.shapes_arrays_metadata.neg_sph_cubes_start; i < static_data.shapes_arrays_metadata.neg_sph_cubes_amount + static_data.shapes_arrays_metadata.neg_sph_cubes_start; i++) {
        ...
        if intr.y > 0.0 {
            (*ismd).st_neg_sph_cubes_amount += 1u;
    ...
    (*ismd).st_neg_inf_cubes_start = ish_index;
    ...
    for (var i = static_data.shapes_arrays_metadata.neg_inf_cubes_start; i < static_data.shapes_arrays_metadata.neg_inf_cubes_amount + static_data.shapes_arrays_metadata.neg_inf_cubes_start; i++) {
        ...
        if intr.y > 0.0 {
            (*ismd).st_neg_inf_cubes_amount += 1u;
    ...
    (*ismd).dyn_neg_cubes_start = ish_index;
    ...
    (*ismd).dyn_neg_spheres_start = ish_index;
    ...
    for (var i = dynamic_data.shapes_arrays_metadata.neg_spheres_start; i < dynamic_data.shapes_arrays_metadata.neg_spheres_amount + dynamic_data.shapes_arrays_metadata.neg_spheres_start; i++) {
        ...
        if intr.y > 0.0 {
            (*ismd).dyn_neg_spheres_amount += 1u;
    ...
    (*ismd).dyn_neg_sph_cubes_start = ish_index;
    ...
    for (var i = dynamic_data.shapes_arrays_metadata.neg_sph_cubes_start; i < dynamic_data.shapes_arrays_metadata.neg_sph_cubes_amount + dynamic_data.shapes_arrays_metadata.neg_sph_cubes_start; i++) {
        ...
        if intr.y > 0.0 {
            (*ismd).dyn_neg_sph_cubes_amount += 1u;
    ...
    (*ismd).dyn_neg_inf_cubes_start = ish_index;
    ...
    for (var i = dynamic_data.shapes_arrays_metadata.neg_inf_cubes_start; i < dynamic_data.shapes_arrays_metadata.neg_inf_cubes_amount + dynamic_data.shapes_arrays_metadata.neg_inf_cubes_start; i++) {
        ...
        if intr.y > 0.0 {
            (*ismd).dyn_neg_inf_cubes_amount += 1u;
    ...
    (*ismd).player_forms_start = ish_index;
...
fn apply_material(
    ...
    let normal = get_normal(pos + ray_dir * dist, ismd, ish);
...
fn fs_main(inn: VertexOutput) -> @location(0) vec4<f32> {
    ...
    var ismd: IntersectedShapesMetadata;
    var ish: array<u32, 16>;

    let offset = find_intersections(camera_position, ray_direction, &ismd, &ish);
    
    let dist_and_depth: vec2<f32> = ray_march(camera_position, ray_direction, offset, &ismd, &ish); 
    // let dist_and_depth: vec2<f32> = old_ray_march(camera_position, ray_direction); 

    var mats: array<i32, 32>;
    var mats_wieghts: array<f32, 32>;
    var mats_count: u32;

    get_mat(camera_position, ray_direction, dist_and_depth.x, &ismd, &ish, &mats, &mats_wieghts, &mats_count);

    var color = apply_material(camera_position, ray_direction, dist_and_depth.x, &ismd, &ish, mats[0]);

    for (var i = 1u; i < mats_count; i++) {

        let new_color = apply_material(camera_position, ray_direction, dist_and_depth.x, &ismd, &ish, mats[i]);

        color = mix(color, new_color, mats_wieghts[i]);
    }
    ...
    // color += normal_shapes[(*ish)[0]].color;
