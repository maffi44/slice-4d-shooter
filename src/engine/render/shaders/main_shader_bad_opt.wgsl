...
struct Intersections {
    ...
    i_normal_shapes: array<Shape, 8>,
    i_negatives_shapes: array<Shape, 8>,
    i_stickiness_shapes: array<Shape, 8>,
    i_neg_stickiness_shapes: array<Shape, 8>,
    i_shapes_metadata: ShapesMetadata,
    i_dyn_normal_shapes: array<Shape, 8>,
    i_dyn_negatives_shapes: array<Shape, 8>,
    i_dyn_stickiness_shapes: array<Shape, 8>,
    i_dyn_neg_stickiness_shapes: array<Shape, 8>,
    i_dyn_shapes_metadata: ShapesMetadata,
}

// var<private> w_plane_intersected: bool = false;

// var<private> i_normal_shapes: array<Shape, 8>;
// var<private> i_negatives_shapes: array<Shape, 8>;
// var<private> i_stickiness_shapes: array<Shape, 8>;
// var<private> i_neg_stickiness_shapes: array<Shape, 8>;

// var<private> i_shapes_metadata: ShapesMetadata;

// var<private> i_dyn_normal_shapes: array<Shape, 8>;
// var<private> i_dyn_negatives_shapes: array<Shape, 8>;
// var<private> i_dyn_stickiness_shapes: array<Shape, 8>;
// var<private> i_dyn_neg_stickiness_shapes: array<Shape, 8>;

// var<private> (*in).i_dyn_shapes_metadata: ShapesMetadata;
...
// previous fn find_intersections()
//
...
fn find_intersections(ro: vec4<f32>, rdd: vec4<f32>, in: ptr<function,Intersections>) -> f32 {
    ...
    var index = 0u;
    (*in).i_shapes_metadata.s_cubes_start = 0u;
    ...
    for (var i = static_data.shapes_arrays_metadata.s_cubes_start; i < static_data.shapes_arrays_metadata.s_cubes_amount + static_data.shapes_arrays_metadata.s_cubes_start; i++) {
        ...
        if intr.y > 0.0 {
            ...
            (*in).i_stickiness_shapes[index] = stickiness_shapes[i];
            index += 1u;
            // st_stickiness_intersected = true;
            offset = min(offset, intr.x);
        }
    ...
    (*in).i_shapes_metadata.s_cubes_amount = index;
    (*in).i_shapes_metadata.s_spheres_start = index;
    ...
    (*in).i_shapes_metadata.s_spheres_amount = index - (*in).i_shapes_metadata.s_spheres_start;
    (*in).i_shapes_metadata.s_sph_cubes_start = index;
    ...
    (*in).i_shapes_metadata.s_sph_cubes_amount = index - (*in).i_shapes_metadata.s_sph_cubes_start;
    (*in).i_shapes_metadata.s_inf_cubes_start = index;
    ...
    (*in).i_shapes_metadata.s_inf_cubes_amount = index - (*in).i_shapes_metadata.s_inf_cubes_start;
    

    // dynamic stickiness

    index = 0u;
    (*in).i_dyn_shapes_metadata.s_cubes_start = 0u;
    ...
    for (var i = dynamic_data.shapes_arrays_metadata.s_cubes_start; i < dynamic_data.shapes_arrays_metadata.s_cubes_amount + dynamic_data.shapes_arrays_metadata.s_cubes_start; i++) {
        ...
        if intr.y > 0.0 {
            ...
            (*in).i_dyn_stickiness_shapes[index] = dyn_stickiness_shapes[i];
            index += 1u;
            // dyn_stickiness_intersected = true;
            offset = min(offset, intr.x);
        }
    ...
    (*in).i_dyn_shapes_metadata.s_cubes_amount = index;
    (*in).i_dyn_shapes_metadata.s_spheres_start = index;
    ...
    (*in).i_dyn_shapes_metadata.s_spheres_amount = index - (*in).i_dyn_shapes_metadata.s_spheres_start;
    (*in).i_dyn_shapes_metadata.s_sph_cubes_start = index;
    ...
    (*in).i_dyn_shapes_metadata.s_sph_cubes_amount = index - (*in).i_dyn_shapes_metadata.s_sph_cubes_start;
    (*in).i_dyn_shapes_metadata.s_inf_cubes_start = index;
    ...
    (*in).i_dyn_shapes_metadata.s_inf_cubes_amount = index - (*in).i_dyn_shapes_metadata.s_inf_cubes_start;


    // static normal shapes

    index = 0u;
    (*in).i_shapes_metadata.cubes_start = 0u;
    ...
    for (var i = static_data.shapes_arrays_metadata.cubes_start; i < static_data.shapes_arrays_metadata.cubes_amount + static_data.shapes_arrays_metadata.cubes_start; i++) {
        ...
        if intr.y > 0.0 {
            ...
            (*in).i_normal_shapes[index] = normal_shapes[i];
            index += 1u;
            // st_noramls_intersected = true;
            offset = min(offset, intr.x);
        }
    ...
    (*in).i_shapes_metadata.cubes_amount = index;
    (*in).i_shapes_metadata.spheres_start = index;
    ...
    (*in).i_shapes_metadata.spheres_amount = index - (*in).i_shapes_metadata.spheres_start;
    (*in).i_shapes_metadata.sph_cubes_start = index;
    ...
    (*in).i_shapes_metadata.sph_cubes_amount = index - (*in).i_shapes_metadata.sph_cubes_start;
    (*in).i_shapes_metadata.inf_cubes_start = index;
    ...
    (*in).i_shapes_metadata.inf_cubes_amount = index - (*in).i_shapes_metadata.inf_cubes_start;

    // dynamic normals 

    index = 0u;
    (*in).i_dyn_shapes_metadata.cubes_start = 0u;
    ...
    for (var i = dynamic_data.shapes_arrays_metadata.cubes_start; i < dynamic_data.shapes_arrays_metadata.cubes_amount + dynamic_data.shapes_arrays_metadata.cubes_start; i++) {
        ...
        if intr.y > 0.0 {
            ...
            (*in).i_dyn_normal_shapes[index] = dyn_normal_shapes[i];
            index += 1u;
            // dyn_noramls_intersected = true;
            offset = min(offset, intr.x);
        }
    ...
    (*in).i_dyn_shapes_metadata.cubes_amount = index;
    (*in).i_dyn_shapes_metadata.spheres_start = index;
    ...
    (*in).i_dyn_shapes_metadata.spheres_amount = index - (*in).i_dyn_shapes_metadata.spheres_start;
    (*in).i_dyn_shapes_metadata.sph_cubes_start = index;
    ...
    (*in).i_dyn_shapes_metadata.sph_cubes_amount = index - (*in).i_dyn_shapes_metadata.sph_cubes_start;
    (*in).i_dyn_shapes_metadata.inf_cubes_start = index;
    ...
    (*in).i_dyn_shapes_metadata.inf_cubes_amount = index - (*in).i_dyn_shapes_metadata.inf_cubes_start;
    ...
    index = 0u;
    (*in).i_shapes_metadata.s_neg_cubes_start = 0u;
    ...
    for (var i = static_data.shapes_arrays_metadata.s_neg_cubes_start; i < static_data.shapes_arrays_metadata.s_neg_cubes_amount + static_data.shapes_arrays_metadata.s_neg_cubes_start; i++) {
        ...
        if intr.y > 0.0 {
            ...
            (*in).i_neg_stickiness_shapes[index] = neg_stickiness_shapes[i];
            index += 1u;
    ...
    (*in).i_shapes_metadata.s_neg_cubes_amount = index;
    (*in).i_shapes_metadata.s_neg_spheres_start = index;
    ...
    (*in).i_shapes_metadata.s_neg_spheres_amount = index - (*in).i_shapes_metadata.s_neg_spheres_start;
    (*in).i_shapes_metadata.s_neg_sph_cubes_start = index;
    ...
    (*in).i_shapes_metadata.s_neg_sph_cubes_amount = index - (*in).i_shapes_metadata.s_neg_sph_cubes_start;
    (*in).i_shapes_metadata.s_neg_inf_cubes_start = index;
    ...
    (*in).i_shapes_metadata.s_neg_inf_cubes_amount = index - (*in).i_shapes_metadata.s_neg_inf_cubes_start;


    // dynamic negative stickiness shapes

    index = 0u;
    (*in).i_dyn_shapes_metadata.s_neg_cubes_start = 0u;
    ...
    for (var i = dynamic_data.shapes_arrays_metadata.s_neg_cubes_start; i < dynamic_data.shapes_arrays_metadata.s_neg_cubes_amount + dynamic_data.shapes_arrays_metadata.s_neg_cubes_start; i++) {
        ...
        if intr.y > 0.0 {
            ...
            (*in).i_dyn_neg_stickiness_shapes[index] = dyn_neg_stickiness_shapes[i];
            index += 1u;
    ...
    (*in).i_dyn_shapes_metadata.s_neg_cubes_amount = index;
    (*in).i_dyn_shapes_metadata.s_neg_spheres_start = index;
    ...
    (*in).i_dyn_shapes_metadata.s_neg_spheres_amount = index - (*in).i_dyn_shapes_metadata.s_neg_spheres_start;
    (*in).i_dyn_shapes_metadata.s_neg_sph_cubes_start = index;
    ...
    (*in).i_dyn_shapes_metadata.s_neg_sph_cubes_amount = index - (*in).i_dyn_shapes_metadata.s_neg_sph_cubes_start;
    (*in).i_dyn_shapes_metadata.s_neg_inf_cubes_start = index;
    ...
    (*in).i_dyn_shapes_metadata.s_neg_inf_cubes_amount = index - (*in).i_dyn_shapes_metadata.s_neg_inf_cubes_start;

    // static negative shapes

    index = 0u;
    (*in).i_shapes_metadata.neg_cubes_start = 0u;
    ...
    for (var i = static_data.shapes_arrays_metadata.neg_cubes_start; i < static_data.shapes_arrays_metadata.neg_cubes_amount + static_data.shapes_arrays_metadata.neg_cubes_start; i++) {
        ...
        if intr.y > 0.0 {
            ...
            (*in).i_negatives_shapes[index] = negatives_shapes[i];
            index += 1u;
    ...
    (*in).i_shapes_metadata.neg_cubes_amount = index;
    (*in).i_shapes_metadata.neg_spheres_start = index;
    ...
    (*in).i_shapes_metadata.neg_spheres_amount = index - (*in).i_shapes_metadata.neg_spheres_start;
    (*in).i_shapes_metadata.neg_sph_cubes_start = index;
    ...
    (*in).i_shapes_metadata.neg_sph_cubes_amount = index - (*in).i_shapes_metadata.neg_sph_cubes_start;
    (*in).i_shapes_metadata.neg_inf_cubes_start = index;
    ...
    (*in).i_shapes_metadata.neg_inf_cubes_amount = index - (*in).i_shapes_metadata.neg_inf_cubes_start;


    // dynamic negative shapes

    index = 0u;
    (*in).i_dyn_shapes_metadata.neg_cubes_start = 0u;
    ...
    for (var i = dynamic_data.shapes_arrays_metadata.neg_cubes_start; i < dynamic_data.shapes_arrays_metadata.neg_cubes_amount + dynamic_data.shapes_arrays_metadata.neg_cubes_start; i++) {
        ...
        if intr.y > 0.0 {
            ...
            (*in).i_dyn_negatives_shapes[index] = dyn_negatives_shapes[i];
            index += 1u;
    ...
    (*in).i_dyn_shapes_metadata.neg_cubes_amount = index;
    (*in).i_dyn_shapes_metadata.neg_spheres_start = index;
    ...
    (*in).i_dyn_shapes_metadata.neg_spheres_amount = index - (*in).i_dyn_shapes_metadata.neg_spheres_start;
    (*in).i_dyn_shapes_metadata.neg_sph_cubes_start = index;
    ...
    (*in).i_dyn_shapes_metadata.neg_sph_cubes_amount = index - (*in).i_dyn_shapes_metadata.neg_sph_cubes_start;
    (*in).i_dyn_shapes_metadata.neg_inf_cubes_start = index;
    ...
    (*in).i_dyn_shapes_metadata.neg_inf_cubes_amount = index - (*in).i_dyn_shapes_metadata.neg_inf_cubes_start;
    ...
    for (var i = 0u; i < dynamic_data.player_forms_amount; i++) {
        ...
        if intr.y > 0.0 {
            (*in).player_forms_intersected = true;
            offset = min(offset, intr.x);
        }
    }

    (*in).player_forms_intersected = false;
...
fn map(p: vec4<f32>, in: ptr<function,Intersections>) -> f32 {
    ...
    // if st_noramls_intersected {
        for (var i = (*in).i_shapes_metadata.cubes_start; i < (*in).i_shapes_metadata.cubes_amount + (*in).i_shapes_metadata.cubes_start; i++) {
            d = min(d, sd_box(p - (*in).i_normal_shapes[i].pos, (*in).i_normal_shapes[i].size) - (*in).i_normal_shapes[i].roundness);
        }
        for (var i = (*in).i_shapes_metadata.spheres_start; i < (*in).i_shapes_metadata.spheres_amount + (*in).i_shapes_metadata.spheres_start; i++) {
            d = min(d, sd_sphere(p - (*in).i_normal_shapes[i].pos, (*in).i_normal_shapes[i].size.x) - (*in).i_normal_shapes[i].roundness);
        }
        for (var i = (*in).i_shapes_metadata.sph_cubes_start; i < (*in).i_shapes_metadata.sph_cubes_amount + (*in).i_shapes_metadata.sph_cubes_start; i++) {
            d = min(d, sd_sph_box(p - (*in).i_normal_shapes[i].pos, (*in).i_normal_shapes[i].size) - (*in).i_normal_shapes[i].roundness);
        }
        for (var i = (*in).i_shapes_metadata.inf_cubes_start; i < (*in).i_shapes_metadata.inf_cubes_amount + (*in).i_shapes_metadata.inf_cubes_start; i++) {
            d = min(d, sd_inf_box(p - (*in).i_normal_shapes[i].pos, (*in).i_normal_shapes[i].size.xyz) - (*in).i_normal_shapes[i].roundness);
        }
    // }

    // dynamic normal shapes
    // if dyn_noramls_intersected {
        for (var i = (*in).i_dyn_shapes_metadata.cubes_start; i < (*in).i_dyn_shapes_metadata.cubes_amount + (*in).i_dyn_shapes_metadata.cubes_start; i++) {
            d = min(d, sd_box(p - (*in).i_dyn_normal_shapes[i].pos, (*in).i_dyn_normal_shapes[i].size) - (*in).i_dyn_normal_shapes[i].roundness);
        }
        for (var i = (*in).i_dyn_shapes_metadata.spheres_start; i < (*in).i_dyn_shapes_metadata.spheres_amount + (*in).i_dyn_shapes_metadata.spheres_start; i++) {
            d = min(d, sd_sphere(p - (*in).i_dyn_normal_shapes[i].pos, (*in).i_dyn_normal_shapes[i].size.x) - (*in).i_dyn_normal_shapes[i].roundness);
        }
        for (var i = (*in).i_dyn_shapes_metadata.sph_cubes_start; i < (*in).i_dyn_shapes_metadata.sph_cubes_amount + (*in).i_dyn_shapes_metadata.sph_cubes_start; i++) {
            d = min(d, sd_sph_box(p - (*in).i_dyn_normal_shapes[i].pos, (*in).i_dyn_normal_shapes[i].size) - (*in).i_dyn_normal_shapes[i].roundness);
        }
        for (var i = (*in).i_dyn_shapes_metadata.inf_cubes_start; i < (*in).i_dyn_shapes_metadata.inf_cubes_amount + (*in).i_dyn_shapes_metadata.inf_cubes_start; i++) {
            d = min(d, sd_inf_box(p - (*in).i_dyn_normal_shapes[i].pos, (*in).i_dyn_normal_shapes[i].size.xyz) - (*in).i_dyn_normal_shapes[i].roundness);
        }
    // }

    // static stickiness shapes
    // if st_stickiness_intersected {
        for (var i = (*in).i_shapes_metadata.s_cubes_start; i < (*in).i_shapes_metadata.s_cubes_amount + (*in).i_shapes_metadata.s_cubes_start; i++) {
            d = smin(d, sd_box(p - (*in).i_stickiness_shapes[i].pos, (*in).i_stickiness_shapes[i].size) - (*in).i_stickiness_shapes[i].roundness, static_data.stickiness);
        }
        for (var i = (*in).i_shapes_metadata.s_spheres_start; i < (*in).i_shapes_metadata.s_spheres_amount + (*in).i_shapes_metadata.s_spheres_start; i++) {
            d = smin(d, sd_sphere(p - (*in).i_stickiness_shapes[i].pos, (*in).i_stickiness_shapes[i].size.x) - (*in).i_stickiness_shapes[i].roundness, static_data.stickiness);
        }
        for (var i = (*in).i_shapes_metadata.s_sph_cubes_start; i < (*in).i_shapes_metadata.s_sph_cubes_amount + (*in).i_shapes_metadata.s_sph_cubes_start; i++) {
            d = smin(d, sd_sph_box(p - (*in).i_stickiness_shapes[i].pos, (*in).i_stickiness_shapes[i].size) - (*in).i_stickiness_shapes[i].roundness, static_data.stickiness);
        }
        for (var i = (*in).i_shapes_metadata.s_inf_cubes_start; i < (*in).i_shapes_metadata.s_inf_cubes_amount + (*in).i_shapes_metadata.s_inf_cubes_start; i++) {
            d = smin(d, sd_inf_box(p - (*in).i_stickiness_shapes[i].pos, (*in).i_stickiness_shapes[i].size.xyz) - (*in).i_stickiness_shapes[i].roundness, static_data.stickiness);
        }
    // }

    // dynamic stickiness
    // if dyn_stickiness_intersected {
        for (var i = (*in).i_dyn_shapes_metadata.s_cubes_start; i < (*in).i_dyn_shapes_metadata.s_cubes_amount + (*in).i_dyn_shapes_metadata.s_cubes_start; i++) {
            d = smin(d, sd_box(p - (*in).i_dyn_stickiness_shapes[i].pos, (*in).i_dyn_stickiness_shapes[i].size) - (*in).i_dyn_stickiness_shapes[i].roundness, static_data.stickiness);
        }
        for (var i = (*in).i_dyn_shapes_metadata.s_spheres_start; i < (*in).i_dyn_shapes_metadata.s_spheres_amount + (*in).i_dyn_shapes_metadata.s_spheres_start; i++) {
            d = smin(d, sd_sphere(p - (*in).i_dyn_stickiness_shapes[i].pos, (*in).i_dyn_stickiness_shapes[i].size.x) - (*in).i_dyn_stickiness_shapes[i].roundness, static_data.stickiness);
        }
        for (var i = (*in).i_dyn_shapes_metadata.s_sph_cubes_start; i < (*in).i_dyn_shapes_metadata.s_sph_cubes_amount + (*in).i_dyn_shapes_metadata.s_sph_cubes_start; i++) {
            d = smin(d, sd_sph_box(p - (*in).i_dyn_stickiness_shapes[i].pos, (*in).i_dyn_stickiness_shapes[i].size) - (*in).i_dyn_stickiness_shapes[i].roundness, static_data.stickiness);
        }
        for (var i = (*in).i_dyn_shapes_metadata.s_inf_cubes_start; i < (*in).i_dyn_shapes_metadata.s_inf_cubes_amount + (*in).i_dyn_shapes_metadata.s_inf_cubes_start; i++) {
            d = smin(d, sd_inf_box(p - (*in).i_dyn_stickiness_shapes[i].pos, (*in).i_dyn_stickiness_shapes[i].size.xyz) - (*in).i_dyn_stickiness_shapes[i].roundness, static_data.stickiness);
        }
    // }

    // static negative shapes
    // if st_negative_intersected {
        for (var i = (*in).i_shapes_metadata.neg_cubes_start; i < (*in).i_shapes_metadata.neg_cubes_amount + (*in).i_shapes_metadata.neg_cubes_start; i++) {
            d = max(d, -(sd_box(p - (*in).i_negatives_shapes[i].pos, (*in).i_negatives_shapes[i].size) - (*in).i_negatives_shapes[i].roundness));
        }
        for (var i = (*in).i_shapes_metadata.neg_spheres_start; i < (*in).i_shapes_metadata.neg_spheres_amount + (*in).i_shapes_metadata.neg_spheres_start; i++) {
            d = max(d, -(sd_sphere(p - (*in).i_negatives_shapes[i].pos, (*in).i_negatives_shapes[i].size.x) - (*in).i_negatives_shapes[i].roundness));
        }
        for (var i = (*in).i_shapes_metadata.neg_sph_cubes_start; i < (*in).i_shapes_metadata.neg_sph_cubes_amount + (*in).i_shapes_metadata.neg_sph_cubes_start; i++) {
            d = max(d, -(sd_sph_box(p - (*in).i_negatives_shapes[i].pos, (*in).i_negatives_shapes[i].size) - (*in).i_negatives_shapes[i].roundness));
        }
        for (var i = (*in).i_shapes_metadata.neg_inf_cubes_start; i < (*in).i_shapes_metadata.neg_inf_cubes_amount + (*in).i_shapes_metadata.neg_inf_cubes_start; i++) {
            d = max(d, -(sd_inf_box(p - (*in).i_negatives_shapes[i].pos, (*in).i_negatives_shapes[i].size.xyz) - (*in).i_negatives_shapes[i].roundness));
        }
    // }

    // dynamic negative shapes
    // if dyn_negative_intersected {
        for (var i = (*in).i_dyn_shapes_metadata.neg_cubes_start; i < (*in).i_dyn_shapes_metadata.neg_cubes_amount + (*in).i_dyn_shapes_metadata.neg_cubes_start; i++) {
            d = max(d, -(sd_box(p - (*in).i_dyn_negatives_shapes[i].pos, (*in).i_dyn_negatives_shapes[i].size) - (*in).i_dyn_negatives_shapes[i].roundness));
        }
        for (var i = (*in).i_dyn_shapes_metadata.neg_spheres_start; i < (*in).i_dyn_shapes_metadata.neg_spheres_amount + (*in).i_dyn_shapes_metadata.neg_spheres_start; i++) {
            d = max(d, -(sd_sphere(p - (*in).i_dyn_negatives_shapes[i].pos, (*in).i_dyn_negatives_shapes[i].size.x) - (*in).i_dyn_negatives_shapes[i].roundness));
        }
        for (var i = (*in).i_dyn_shapes_metadata.neg_sph_cubes_start; i < (*in).i_dyn_shapes_metadata.neg_sph_cubes_amount + (*in).i_dyn_shapes_metadata.neg_sph_cubes_start; i++) {
            d = max(d, -(sd_sph_box(p - (*in).i_dyn_negatives_shapes[i].pos, (*in).i_dyn_negatives_shapes[i].size) - (*in).i_dyn_negatives_shapes[i].roundness));
        }
        for (var i = (*in).i_dyn_shapes_metadata.neg_inf_cubes_start; i < (*in).i_dyn_shapes_metadata.neg_inf_cubes_amount + (*in).i_dyn_shapes_metadata.neg_inf_cubes_start; i++) {
            d = max(d, -(sd_inf_box(p - (*in).i_dyn_negatives_shapes[i].pos, (*in).i_dyn_negatives_shapes[i].size.xyz) - (*in).i_dyn_negatives_shapes[i].roundness));
        }
    // }
    ...
    // if st_neg_stickiness_intersected {
        for (var i = (*in).i_shapes_metadata.s_neg_cubes_start; i < (*in).i_shapes_metadata.s_neg_cubes_amount + (*in).i_shapes_metadata.s_neg_cubes_start; i++) {
            d = smax(d, -(sd_box(p - (*in).i_neg_stickiness_shapes[i].pos, (*in).i_neg_stickiness_shapes[i].size) - (*in).i_neg_stickiness_shapes[i].roundness), static_data.stickiness);
        }
        for (var i = (*in).i_shapes_metadata.s_neg_spheres_start; i < (*in).i_shapes_metadata.s_neg_spheres_amount + (*in).i_shapes_metadata.s_neg_spheres_start; i++) {
            d = smax(d, -(sd_sphere(p - (*in).i_neg_stickiness_shapes[i].pos, (*in).i_neg_stickiness_shapes[i].size.x) - (*in).i_neg_stickiness_shapes[i].roundness), static_data.stickiness);
        }
        for (var i = (*in).i_shapes_metadata.s_neg_sph_cubes_start; i < (*in).i_shapes_metadata.s_neg_sph_cubes_amount + (*in).i_shapes_metadata.s_neg_sph_cubes_start; i++) {
            d = smax(d, -(sd_sph_box(p - (*in).i_neg_stickiness_shapes[i].pos, (*in).i_neg_stickiness_shapes[i].size) - (*in).i_neg_stickiness_shapes[i].roundness), static_data.stickiness);
        }
        for (var i = (*in).i_shapes_metadata.s_neg_inf_cubes_start; i < (*in).i_shapes_metadata.s_neg_inf_cubes_amount + (*in).i_shapes_metadata.s_neg_inf_cubes_start; i++) {
            d = smax(d, -(sd_inf_box(p - (*in).i_neg_stickiness_shapes[i].pos, (*in).i_neg_stickiness_shapes[i].size.xyz) - (*in).i_neg_stickiness_shapes[i].roundness), static_data.stickiness);
        }
        // d = max(d, -dd);
    // }
    ...
    // if dyn_neg_stickiness_intersected {
        ...
        for (var i = (*in).i_dyn_shapes_metadata.s_neg_cubes_start; i < (*in).i_dyn_shapes_metadata.s_neg_cubes_amount + (*in).i_dyn_shapes_metadata.s_neg_cubes_start; i++) {
            d = smax(d, -(sd_box(p - (*in).i_dyn_neg_stickiness_shapes[i].pos, (*in).i_dyn_neg_stickiness_shapes[i].size) - (*in).i_dyn_neg_stickiness_shapes[i].roundness), static_data.stickiness);
        }
        for (var i = (*in).i_dyn_shapes_metadata.s_neg_spheres_start; i < (*in).i_dyn_shapes_metadata.s_neg_spheres_amount + (*in).i_dyn_shapes_metadata.s_neg_spheres_start; i++) {
            d = smax(d, -(sd_sphere(p - (*in).i_dyn_neg_stickiness_shapes[i].pos, (*in).i_dyn_neg_stickiness_shapes[i].size.x) - (*in).i_dyn_neg_stickiness_shapes[i].roundness), static_data.stickiness);
        }
        for (var i = (*in).i_dyn_shapes_metadata.s_neg_sph_cubes_start; i < (*in).i_dyn_shapes_metadata.s_neg_sph_cubes_amount + (*in).i_dyn_shapes_metadata.s_neg_sph_cubes_start; i++) {
            d = smax(d, -(sd_sph_box(p - (*in).i_dyn_neg_stickiness_shapes[i].pos, (*in).i_dyn_neg_stickiness_shapes[i].size) - (*in).i_dyn_neg_stickiness_shapes[i].roundness), static_data.stickiness);
        }
        for (var i = (*in).i_dyn_shapes_metadata.s_neg_inf_cubes_start; i < (*in).i_dyn_shapes_metadata.s_neg_inf_cubes_amount + (*in).i_dyn_shapes_metadata.s_neg_inf_cubes_start; i++) {
            d = smax(d, -(sd_inf_box(p - (*in).i_dyn_neg_stickiness_shapes[i].pos, (*in).i_dyn_neg_stickiness_shapes[i].size.xyz) - (*in).i_dyn_neg_stickiness_shapes[i].roundness), static_data.stickiness);
        }
        // d = max(d, -ddd);
    // }

    if (*in).player_forms_intersected {
        var dddd = MAX_DIST;
        for (var i = 0u; i < dynamic_data.player_forms_amount; i++) {
...
fn get_mats(
    ...
    // if st_noramls_intersected {
        for (var i = (*in).i_shapes_metadata.cubes_start; i < (*in).i_shapes_metadata.cubes_amount + (*in).i_shapes_metadata.cubes_start; i++) {
            let shape = (*in).i_normal_shapes[i];
    ...
    // if dyn_noramls_intersected {
        for (var i = (*in).i_dyn_shapes_metadata.cubes_start; i < (*in).i_dyn_shapes_metadata.cubes_amount + (*in).i_dyn_shapes_metadata.cubes_start; i++) {
            let shape = (*in).i_dyn_normal_shapes[i];
    ...
    // if st_stickiness_intersected {
        for (var i = (*in).i_shapes_metadata.s_cubes_start; i < (*in).i_shapes_metadata.s_cubes_amount + (*in).i_shapes_metadata.s_cubes_start; i++) {
            let shape = (*in).i_stickiness_shapes[i];
            let dd = sd_box(p - shape.pos, shape.size) - shape.roundness;
    ...
    // if dyn_stickiness_intersected {
        for (var i = (*in).i_dyn_shapes_metadata.s_cubes_start; i < (*in).i_dyn_shapes_metadata.s_cubes_amount + (*in).i_dyn_shapes_metadata.s_cubes_start; i++) {
            let shape = (*in).i_dyn_stickiness_shapes[i];
            let dd = sd_box(p - shape.pos, shape.size) - shape.roundness;
...
fn get_shadow(ray_origin_base: vec4<f32>, ray_direction: vec4<f32>, in: ptr<function, Intersections>) -> f32 {

    let offset = find_intersections(ray_origin_base, ray_direction, in);
...
fn apply_material(
    ...
    let normal = get_normal(hited_pos, in);
    let next_normal = get_normal(hited_pos+ray_dir*MIN_DIST*4.8, in);
    ...
    if static_data.shadows_enabled == 1 {
        sun_shadow_1 = get_shadow(hited_pos + normal*MIN_DIST*2.0, sun_dir_1, in);
    }
...
fn fs_main(inn: VertexOutput) -> @location(0) vec4<f32> {
    ...
    in.w_plane_intersected = false;
    in.player_forms_intersected = false;

    let offset = find_intersections(camera_position, ray_direction, &in);
    let dist_and_depth: vec2<f32> = ray_march(camera_position, ray_direction, offset, &in); 


    var mats = get_mats(camera_position, ray_direction, dist_and_depth.x, &in);
    var color = apply_material(camera_position, ray_direction, dist_and_depth.x, mats.materials[0], &in);

    for (var i = 1u; i < mats.materials_count; i++) {
        let new_color = apply_material(camera_position, ray_direction, dist_and_depth.x, mats.materials[i], &in);
