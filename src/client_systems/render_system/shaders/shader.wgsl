...
fn vs_main(
    ...
    out.coordinates = model.coordinates;
...
// Fragment shader
/*
@location(0) aspect: f32;
@location(1) camera_position: vec3<f32>;
@location(2) rotation_matrix: mat3<f32>;
@location(3) resolution: vec3<f32>;
@location(4) time: f32;
@location(5) iTimeDelta: f32;
@location(6) iFrame: i32;
@location(7) iFrameRate: f32;
@location(8) iMouse: vec4<f32>;
// uniform f32 aspect;
...
fn sd_box(p: vec4<f32>, b: vec3<f32>) -> f32 {
...
fn sd_octahedron(p: vec4<f32>, s: f32) -> f32 {
    p = abs(p);
...
fn map(p: vec4<f32>) -> f32 {
    var d: f32 = sd_inf_box(p - vec4<f32>(0, 0, 0, 0), vec3<f32>(3, 0.1, 3));
...
fn get_normal(p: vec4<f32>) -> vec3<f32> {
    ...
    var a: vec3<f32> = p + e.yxxz;
    var b: vec3<f32> = p + e.xyxz;
    var c: vec3<f32> = p + e.xxyz;
    var d: vec3<f32> = p + e.yyyz;
...
fn ray_march(ray_origin: vec4<f32>, ray_direction: vec4<f32> ) -> vec2<f32>  {
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    var uv: vec2<f32> = (VertexOutput.coordinates / resolution.xy - 0.5) * 2.;
    uv.x *= aspect;

    var ray_direction: vec3<f32> = normalize(vec3<f32>(uv, 1.));
    ray_direction *= rotation_matrix;

    var cam_pos: vec4<f32> = vec4<f32>(camera_position, camera_position.y * 0.8);

    var dist_and_depth: vec2<f32> = ray_march(cam_pos, vec4<f32>(ray_direction, 0.0)); 

    var normal: vec3<f32> = get_normal(dist_and_depth.x * vec4<f32>(ray_direction, 0.0) + cam_pos);
